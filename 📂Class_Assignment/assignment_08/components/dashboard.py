...
class SensorManager:
    ...
    def __init__(self):
        ...
        self.history = st.session_state.sensor_history
...
class WeatherManager:
    ...
    def __init__(self, default_city="Karachi"):
        self.default_city = default_city if st.session_state.language == "en" else "کراچی"
...
class DashboardApp:
    """Main application class for rendering the Smart Irrigation Dashboard."""
...
# Example usage (assuming farm_manager and crops are provided)
if __name__ == "__main__":
    # Placeholder for farm_manager and crops (these should be initialized as in your app)
    from models.farm_manager import FarmManager
    from models.crop import Crop
    crops = [
        Crop(name="Tomato", sowing_months=[3, 4], harvesting_months=[7, 8], tasks={"Spraying": 60}, seasonal_suitability={"spring": 0.9}, health_thresholds={"moisture": (50, 70)}, weather_sensitivity={"heavy_rain": "Improve drainage"}),
        Crop(name="Rice", sowing_months=[6, 7], harvesting_months=[10, 11], tasks={"Weeding": 40}, seasonal_suitability={"monsoon": 0.95}, health_thresholds={"moisture": (60, 80)}, weather_sensitivity={"heavy_rain": "Improve drainage"}),
    ]
    farm_manager = FarmManager(crops)
    
    # Initialize and render the app
    app = DashboardApp(farm_manager, crops)
    app.render_dashboard()  # or app.render_home() depending on the page
